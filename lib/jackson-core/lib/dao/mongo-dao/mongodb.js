...
class MongoDb {
  ...
  getRipWhitelisted(host) {
    return new Promise((resolve, reject) => {
      if (!host) throw new Error('Cannot get if rip is whitelisted. Must have a Host.');

      this.ensureConnection()
        .then(() => {
          WhitelistedDomain.findOne({ name: { $regex: host, $options: 'i' } })
            .then((whitelistedRecord) => {
              if (!whitelistedRecord) resolve(false);
              else {
                logger.silly(`MongoDb: getRipWhitelisted: ${host} is whitelisted`);
                resolve(true);
              }
            })
            .catch((err) => {
      ...
      this.ensureConnection()
        .then(() => {
          ...
          this.getUserFromUUID(rip.uuid)
            .then((user) => {
              this.getRipWhitelisted(rip.host)
                .then((whitelisted) => {
