...
class MongoDb {
  ...
  removeWhitelistedDomain(name) {
    return new Promise((resolve, reject) => {
      this.ensureConnection()
        .then(() => {
          WhitelistedDomain.deleteOne({ name })
            .then((response) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ensureConnection()
        .then(() => Rip.findOneAndUpdate({
          ...
          $set: _.omit(rip, 'offer', 'take_rate', 'miningConfig', 'currentOffer'),
        }, {
          upsert: true,
        }))
        .then(() => resolve())
      ...
      if (_.isEmpty(rips)) {
        ...
        .then(() => {
          ...
          _.each(rips, (rip) => {
            ...
            }).updateOne({ $set: _.omit(rip, ['_id', 'offer', 'take_rate']) });
          });
