  ...
  ensureConnection() {
    return new Promise((resolve, reject) => {
      if (this.db.connected) {
        ...
        logger.info('RedisDb: No cached connection!');
        this.db = redis.createClient(this.config);
        logger.info(`redis connection: ${this.db.connected}`);
        this.db.on('ready', () => {
          resolve();
        });
    ...
    return new Promise((resolve, reject) => {
      this.ensureConnection()
        .then(() => {
          ...
          this.db.eval(luaQueryUpdateShouldPersist(
            rip,
            this.ripLastPersistedKey,
            upperLimitToPersist,
            newTime,
          ), 0, (err, shouldPersist) => {
    ...
    return new Promise((resolve, reject) => {
      this.ensureConnection()
        .then(() => {
          ...
          this.db.eval(luaQueryOutdatedAndPurge(
            this.ripLastPersistedKey, upperLimitToPersist), 0, (err, outOfDateRips) => {
              if (err) {
                logger.silly('queryRecordsToPersistAndRemoveThem: error');
                logger.silly(this.ripLastPersistedKey);
                logger.silly(upperLimitToPersist);
                logger.silly(err);
                reject(err);
              } else {
                logger.silly('queryRecordsToPersistAndRemoveThem: no error');
                logger.silly(outOfDateRips);
          ...
          this.db.watch(url, (error) => {
            if (error) reject(error);
              ...
              this.db.get(url, (err, getReply) => {
                logger.silly(`RedisDb: getRipAndWatch: getting rip for url: ${url}`);
                if (err || _.isNull(getReply)) {
                  // Only watch if we have something
                  // to watch. If we didn't get a rip
                  // we don't care if we watch it.
                  this.db.unwatch(url, () => {
                    reject(err);
                  });
                } else {
                  logger.silly(`RedisDb: getRipAndWatch: got rip for ${url}: ${JSON.stringify(getReply)}`);
