    ...
    return new Promise((resolve, reject) => {
      ...
      this.updateRipInRedisWithRetry({
        ...
        .then((recordThatWasWrittenToRedis) => {
          console.log(recordThatWasWrittenToRedis);
          // TODO: update the current update sorted set to query & update not just update.
          // for new rips this means the query will just be null. then i can just use that
          // method here.

          // Compare now() - that time
          // to see if the window is too large. If it is persist it to mongo then resolve
          // else just resolve now.
