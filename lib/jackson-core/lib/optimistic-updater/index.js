...
class OptimisticUpdater {
  constructor({ geo, userConfig }) {
    ...
    this.getDailyJacks = hourly =>
      _.reduce(hourly, (totalSum, hourOfHits) =>
        totalSum + _.reduce(hourOfHits.hits, (hourlySum, hit) => hourlySum + hit.jacks, 0), 0);
  }
  ...
  updateRip(ripRecord) {
    ...
    // daily_hits & daily_jacks needs post archive processing values.
    return {
      ...response,
      daily_hits: this.getDailyHits(response.archive.hourly),
      daily_jacks: this.getDailyJacks(response.archive.hourly),
    };
  }
}
