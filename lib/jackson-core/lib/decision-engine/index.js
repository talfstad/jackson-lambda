...
class DecisionEngine {
  constructor() {
    ...
    this.checkClientNotWhitelisted = decisionInformation =>
      new Promise((resolve, reject) => {
        const { ip } = decisionInformation.inputs;

        this.db.checkClientNotWhitelisted(ip)
          .then(() => resolve())
  ...
  decideIfJack(decisionInformation) {
    return new Promise((resolve, reject) => {
      if (
        ...
        this.checkClientNotWhitelisted(decisionInformation)
        .then(() => resolve(this.makeJackDecision(decisionInformation)))
        .catch(err => reject(err));
      }
    });
  }
